...
?>
    ...
    <h1>User Answer Table</h1>
    <table>
        <tr><th>id</th><th>question</th><th>response</th><th>correct</th></tr>
        <?php
        try{
            $statement = $dbh->prepare("select * from quizItemResponse");
            $statement->execute();
            $columns = ['id', 'question', 'response','correct'];
            while($row = $statement->fetch(PDO::FETCH_ASSOC)){
                echo "<tr>";
                foreach($columns as $col){
                    echo "<td>${row[$col]}</td>";
