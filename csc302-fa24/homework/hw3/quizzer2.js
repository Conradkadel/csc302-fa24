...
$(document).ready(function(){
    ...
    $(window).on('hashchange', '#check-quiz', updateUI);
...
/**
 * Updates the UI between Admin View and Normal Question View,
 * Problem is that side needs to be refreshed manually
 */
function updateUI(){
    ...
    console.log(id)
    if(id === '#admin'){
        $('body').addClass('quiz-admin-panel').removeClass('quiz-panel')
        document.getElementById('quiz-admin-panel').style.display = 'block'
        document.getElementById('quiz-panel').style.display = 'none'
        console.log('1')
    }
    else{
        $('body').addClass('quiz-panel').removeClass('quiz-admin-panel')
        document.getElementById('quiz-admin-panel').style.display = 'none'
